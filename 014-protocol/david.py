...
for i in range(10):
    m = create_model()
    m.load_weights('\github\emnist\dropout_0.2/weights/{:03d}epochs_weights_model_{}.pkl'.format(weights_epoch, i))
    models.append(m)
###########
eval_batch_size = 512
##############
...
#         h = m.fit(batches, steps_per_epoch=steps_per_epoch, epochs=num_epochs, verbose=0,  # _generator
...
eval_batch_size = 512
###############
#models[0].predict(x_test, batch_size=eval_batch_size)
all_preds = np.stack([m.predict(x_test, batch_size=eval_batch_size) for m in models])
##############
#avg_preds = all_preds.mean(axis=0)
###############
#print((1 - keras.metrics.categorical_accuracy(y_test, avg_preds).eval().mean()) * 100)
###############
