...
app([54,31,82,39,74,46,41,64],"00F01101")
...
for p in range(81):
	...
	for r in range(8):
		...
		s = score(p,r)
		ss += s
		diff = elos[p+1] - elos[opp]
		summa = elos[p+1] + elos[opp]

		#value = f(diff)*summa*s
		#value = f(diff)*2*elos[opp]*s
		#value = f(diff)*s * elos[opp]
		value = (elos[opp]-OFFSET) * s

		total += value

		# print(elos[p+1],elos[opp],diff,s,round(value))
	print('total',elos[p+1],round(total),scores(p)) #,buchholz(p))
	# print("")
